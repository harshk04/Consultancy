...
export function Footer() {
  return (
    <footer
      className="border-t border-brand-gold/25 bg-brand-blue text-white"
      ...
      <Container className="py-14">
        <div className="grid items-start gap-10 md:grid-cols-3">
          <div>
            ...
            <p className="mt-5 text-sm text-white/85">{site.tagline}</p>
            <p className="mt-1 text-sm text-white/85">{site.brandLine}</p>
            <div
              aria-hidden="true"
          ...
          <nav aria-label="Footer" className="min-w-0">
            ...
            <ul className="mt-5 grid gap-2 sm:grid-cols-2">
              {quickLinks.map((item) => (
                <li key={item.href}>
                  <Link
                    className="text-sm text-white/85 transition hover:text-brand-gold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold/60 focus-visible:ring-offset-2 focus-visible:ring-offset-brand-blue"
                    href={item.href}
                  >
                    {item.label}
                  </Link>
          ...
          <div className="min-w-0">
            ...
            <dl className="mt-5 space-y-4 text-sm">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <dt className="text-xs font-medium uppercase tracking-[0.18em] text-white/70">
                    Email
                  </dt>
                  <dd className="mt-1 truncate text-white/90">
        ...
        <div className="mt-12 flex flex-col gap-3 border-t border-brand-gold/20 pt-6 text-xs text-white/75 sm:flex-row sm:items-center sm:justify-between">
          <p>
            © {new Date().getFullYear()} {site.name}. All rights reserved.
          </p>
          <p className="text-white/75">Guiding Minds. Shaping Leaders.</p>
        </div>
      </Container>
    </footer>
  );
